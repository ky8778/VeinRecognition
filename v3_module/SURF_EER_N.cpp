...
int main() {
	...
	for (int hand_index = NUM_IMAGE - 10; hand_index < NUM_IMAGE; hand_index++) {
		printf("HAND %d\n", hand_index);
		if (hand_index < 10) {
			strcpy(ex1_, "00");
		...
		for (int base_index = hand_index+1; base_index <= NUM_IMAGE; base_index++) {
			if (base_index < 10) {
				strcpy(ex2_, "00");
				_itoa(base_index, hand, 10);
				strcat(ex2_, hand);
			}
			else if (base_index != 100) {
				strcpy(ex2_, "0");
				_itoa(base_index, hand, 10);
				strcat(ex2_, hand);
			}
